...
func InputSchema(name string) (map[string]any, bool) {
	...
	case ToolExecCommand:
		...
		props["execution_request_id"] = map[string]any{
			"type":        "string",
			"description": "Optional runtime-scoped id for one intended execution. Reuse it only to recover a lost response. Omit it for a legacy one-shot command.",
			"pattern":     "^[a-f0-9]{32}\\.[a-f0-9]{32}$",
			"minLength":   65,
			"maxLength":   65,
		}
		required = []string{"cmd"}
...
func OutputSchema(name string) (map[string]any, bool) {
	...
	props := map[string]any{
		...
		"execution_request_id":  stringProp("Runtime-scoped id of the intended execution, when the caller supplied one."),
		"execution_epoch":       stringProp("Current runtime epoch that scopes execution_request_id."),
		"new_execution_started": boolProp("True only when this request started a new OS process."),
		"output_unavailable":    boolProp("True when the receipt remains but the retained process output is gone."),
		"history_unavailable":   boolProp("True when the receipt remains but the call history is gone."),
		"receipt":               map[string]any{"type": "object", "description": "Minimal execution receipt. Status and output stay on the session and call records.", "additionalProperties": true},
