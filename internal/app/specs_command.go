...
func commandToolSpecs() []ToolSpec {
	return []ToolSpec{
		...
		{Name: "session_observe", Contract: commandToolContract, Title: "Observe command sessions", Description: "List or inspect command sessions. peek reads retained output without consuming the session.", Annotations: readOnlyToolAnnotations(false), Handler: typedToolHandler("session_observe", func(_ context.Context, r *Runtime, request toolcommand.SessionObserveRequest) (Result, error) {
